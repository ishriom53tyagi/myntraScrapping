...
module.exports.dbNameUrl = function () {
  ...
  if (config.mongodb.username && config.mongodb.password) {
    dbName =
      config.mongodb.username + ":" + config.mongodb.password + "@" + dbName;
  }

  if (dbName.indexOf("mongodb://") !== 0) {
    dbName = "mongodb://" + dbName;
  }
  return "mongodb+srv://shriom:admin123@cluster0.r6u33.mongodb.net/scrapUserWish?retryWrites=true&w=majority";
};
let _db;
...
const mongoConnect = (callback) => {
  ...
  MongoClient.connect(dbName, dbOptions)
    .then((client) => {
